...
class DockerTestContainer:
    ...
    def run(self, param_list: list[str], check_exit_code: bool = True) -> tuple[int, str]:
    ...
    def make_and_upload_file(self, target_path_in_container: PathLike, file_name: str,  content: str) -> Path:
        ...
        tarstream.seek(0)
        res = self.container.put_archive(str(target_path_in_container), tarstream.read())
        assert res
        return Path(target_path_in_container) / file_name

    def list_apt(self):
        _, out = self.run(["bash", "-c", """dpkg-query -W -f='{"package":"${Package}","version":"${Version}"}\n'"""])
        packages = []
