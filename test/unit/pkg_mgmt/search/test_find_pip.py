...
def _make_phase(phase_data: TestPhaseData) -> Phase:
    phase = Phase(
        ...
        tools=Tools(pip=Pip(version="1.2.3")) if phase_data.enable_pip else None,
    )
    return phase
...
def test_find_pip_unique(phases):
    expected_err = r"Found more than one result for pip: [Pip(version='1.2.3', needs_break_system_packages=False, comment=None), Pip(version='1.2.3', needs_break_system_packages=False, comment=None)]"
